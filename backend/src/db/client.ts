...
export function openDatabase(path?: string): AppDatabase {
  ...
  // Map ':memory:' to a shared-cache URI database ('file::memory:?cache=shared') which
  // supports WAL mode and is used exclusively by tests.
  const isMemory = dbPath === ':memory:'
  const resolvedPath = isMemory ? 'file::memory:?cache=shared' : dbPath
